...
```lua
for pos, d in pairs(Battle.Members) do
    local result = RandomSelect(Case)
    local currentSpin = Battle.CurrentSpin[pos]

    if (not table.find(UserIDs, d.Player)) then
        table.insert(UserIDs, d.Player)
    end

    results[pos] = result
    currentSpin.Item = result
    currentSpin.SpinEndTime = SpinEnd
    currentSpin.Case = CaseName
    currentSpin.FinalSpeed = Speed
end
```
